 ...
 * Kept DOM-free so it can be reasoned about independently of the renderer.
 *
 * Ported near-verbatim from Brah (MIT, KenKaiii):
 * src/renderer/realtime-playback.js. Wrapped as a global (window.AcosRealtime)
 * because the AICOS chat renderer loads classic <script> files, not ESM.
 */
(function () {
  window.AcosRealtime = window.AcosRealtime || {};

  function createRealtimePlaybackTracker() {
  ...
  function isBenignCancelError(error) {
    const haystack = `${(error && error.code) || ''} ${(error && error.message) || ''}`.toLowerCase();
    return haystack.includes('cancel') && haystack.includes('active response');
  }

  window.AcosRealtime.createRealtimePlaybackTracker = createRealtimePlaybackTracker;
  window.AcosRealtime.isBenignCancelError = isBenignCancelError;
})();
