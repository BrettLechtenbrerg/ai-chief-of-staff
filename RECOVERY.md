...
> **Latest release**: https://github.com/BrettLechtenbrerg/ai-chief-of-staff/releases/tag/v1.0.0-beta.22 (public, prerelease)
...
> - **The release pipeline is tag-first and manually published.** GitHub Actions uses `npm ci`, full gates, native-module architecture checks, checksums, macOS signing/notarization, Windows Authenticode validation, a manual `publish` input, and a protected `release` environment requiring Brett's approval. Unsigned Windows artifacts cannot publish.
...
>
> **beta.25 Mac-only public prerelease (published August 14, 2026):**
> - Immutable annotated tag `v1.0.0-beta.25` and successful tag workflow run `31603383781` are the sole Mac artifact source. A clean workflow download matches `SHA256SUMS-macos.txt`; the x64 DMG SHA-256 is `66e51d13006bbcae595d8af8d86d53ab4a0f56febf3a50d3357388f741497514` and the arm64 DMG SHA-256 is `3407655c7c3d743f8e6f4327a3d0dd650d67687043a29ac3e0045fdb9153e7d3`.
> - Both bundles pass the repository native-module architecture gate, strict Developer ID verification for team `2HQTY95NHD`, Gatekeeper assessment, and DMG notarization-staple validation. Apple Silicon remains build-gate verified only; no real Apple Silicon device acceptance was performed.
> - Real Intel iMac acceptance passed on August 14 against the exact run-`31603383781` x64 DMG. Beta.25 preserved 18 sessions and 11 facts, completed Claude and GPT typed chat, live dictation, a two-turn interactive Voice session, and a clean normal restart with SQLite integrity `ok` and unchanged session/message/fact counts. The user separately confirmed manual input and Voice were working.
> - The immediate pre-publication SQLite backup is `~/Library/Application Support/ai-chief-of-staff/backups/ai-chief-of-staff-pre-public-beta25-20260814T231844Z.db`; its integrity check passed. `/Applications/AI Chief of Staff beta.23 rollback.app` preserves the prior signed app until beta.25 is stable.
> - `.github/workflows/publish-mac-only-release.yml` is the only approved path for this filtered release. It downloads only the Mac artifact from the immutable run, validates both architectures/signatures/notarization, and requires the protected `release` environment plus an independently verified private draft before public promotion.
> - Windows remains pinned to the public beta.20 x64 installer from release `v1.0.0-beta.22`: SHA-256 `7464181a0dbb60bdce8aa3b9948ba164898b326aff84703c94468cf919c46d6e`; its blockmap SHA-256 is `a05235bed923ada2f4313bd3455389521726dc249d427e39fba0db060e273134`. Generated `latest.yml` contains only that x64 fallback and reports version `1.0.0-beta.20`.
> - No beta.23 or beta.25 Windows executable, ZIP, blockmap, or updater manifest may enter this release. The beta.25 Windows candidate remains deferred for its separate Voice/API-key and model-discovery acceptance work.
> - Protected draft run `31849210095` created and verified the exact 10-asset inventory; protected publish run `31850297453` revalidated the source and promoted release `370865321` at 2026-08-14T23:29:50Z. Public Mac DMGs and the mirrored beta.20 Windows installer return HTTP 200, and public updater manifests retain beta.25 for Mac and beta.20 x64 for Windows. Rollback is to return beta.25 to draft, restore website Mac constants to beta.22, and leave the existing beta.22/beta.20 release untouched.
>
...
|-----|------|-------------|
| `v1.0.0-beta.23` | Aug 11, 2026 | **Twenty-third beta — trust, voice, and Windows recovery.** Removes remote renderer execution, keeps decrypted secrets in the main process, validates trusted IPC senders and canonical paths, enforces mode-aware tool capabilities and user-originated approvals, hardens unattended/MCP/AEO execution, upgrades Realtime voice to `gpt-realtime-2.1` with half-duplex fallback and spoken approve/deny, adds permission-safe rotating SQLite backups, upgrades Electron 43, restores architecture-verified native Windows x64 packaging, and moves production Windows signing to Azure Artifact Signing with protected GitHub OIDC. Signed tag artifacts and real Intel Mac acceptance are verified; public promotion remains blocked only on real Windows x64 acceptance. |
| `v1.0.0-beta.22` | Jul 8, 2026 | **Twenty-second beta — working-indicator fixes for long/silent tool calls (Manny bug report #2) + discovery that the beta.21 native-module guard was dead code in release builds.** Manny reported (Desktop doc `Bug_ Working indicator disappears….docx`) that during long operations — most reproducibly a Remotion video render — the thinking indicator vanished while the agent was still working, surviving the beta.20 spinner fix. His diagnosis verified against the code and found to be THREE compounding bugs: (1) **UI**: `message-renderer.js`'s `partial_text` RAF hides the indicator once the streaming bubble has content, and the only re-show (line ~352) is event-driven — it needs another non-partial_text status event, which never arrives during a silent multi-minute tool call. (2) **The unnamed root cause in his own screenshot** ("wrapper timeout as usual — render's running in background. polling:"): `render_video`/`trim_video_silence`/`scaffold_video_project` were missing from `TOOL_TIMEOUTS` in `src/tools/diagnostics.ts`, so the wrapper's 30s default killed the tool call while the real render ran up to 20 min — forcing the agent into silent background shell polling (the very silence that starved the indicator). (3) No heartbeat: gg-agent passes `ctx.onUpdate` to every tool.execute and chat-engine had a commented-out `tool_call_update` case — plumbing existed, nothing wired. **Fixes:** `3712a59` — stream watchdog in `message-renderer.js`: every partial_text chunk re-arms a 2.5s timer (`streamWatchdogBySession` in state.js); if the stream goes quiet without `done`, the indicator re-shows as 'still working... 🐾'; any real status event clears it; all teardown paths (done branch, `stopQuery`, both messaging.js cleanup blocks) clear it, and the fire-time guard (statusEl exists + isLoading + display:none) makes a stale timer harmless. `c3bdfc3` — wrapper timeouts now OUTLAST internal runInWorkspace timeouts (scaffold 12→13 min, render 20→21, trim 30→31) so tools return their structured result instead of the wrapper racing them out; regression test pins the invariant. `e3231ad` — heartbeat pipeline: `wrapToolHandler` forwards an optional `ToolProgressContext`; chat-tools bridges gg-agent's `context.onUpdate` into it (both Chat + Coder modes); `renderVideo` emits 'rendering <slug>… Ns' every 10s around the blocking call (cleared in finally); chat-engine surfaces `tool_call_update` as a 'thinking' status ('tool_start' would inflate the tool counter every beat). **Live repro before shipping:** real render of the beta.19 OneStepToday comp via a throwaway vitest harness — heartbeat fired at exactly +10.0s, render succeeded (11.6s warm-cache). Suite 1250/1250, typecheck + lint clean. **CRITICAL BUILD-PIPELINE DISCOVERY:** while watching this build's log, `[verify-native-modules] OK` never appeared — the beta.21 guard was chained at the BOTTOM of `build/afterPack.cjs`, but the macOS signing section EARLY-RETURNS when a real Developer ID is configured (i.e. on every `dist:signed`), so the guard only ever ran on unsigned local builds — the ones that never ship. A beta.20-repeat would have sailed through. Fixed (`5614397`): gate moved BEFORE the signing section (after cleanup + symlinks — still the final .node file set; signing never alters .node contents). Because the running build had the old hook cached, both beta.22 packs were verified manually (arm64: 13 modules OK; x64: 13 modules OK). **Release mechanics:** tag → detached-bg `dist:signed`; arm64 DMG notarized+stapled in-pipeline; **x64 DMG notarize failed on a network blip mid-poll** ("Internet connection appears to be offline") — submission `26b31cc4` was already Accepted at Apple, so recovery was `stapler staple` + re-run `patch-latest-mac-yml.cjs` (the beta.16 lesson — stapling changes DMG bytes AFTER the yml was patched). Note: `spctl -a -t open` says 'rejected/no usable signature' on ALL our DMGs incl. shipped beta.21 — normal for this pipeline (inner .app = 'accepted / Notarized Developer ID'; DMG carries the ticket, not a signature). Windows NOT rebuilt — beta.20's healthy `latest.yml` + 3 setup.exe/blockmaps mirrored onto beta.22 (same as beta.21 did). Draft with manifests first, big assets in batches, published — 16 assets, all links verified 200. Landing page bumped (`TSAI-Site@445880a`), verified live serving beta.22 with browser UA. **Open:** Manny's live confirmation on a real long render (visual watchdog check closes his report); agent-performance upgrades re-queued for beta.23; Campaign Ops Phase 2; carried-forward chores. |
| `v1.0.0-beta.21` | Jul 7, 2026 | **Twenty-first beta — CRITICAL Apple Silicon brick fix + build-pipeline guard (incident response, same-day).** Manny reported (screenshots + Zoom) that after updating, chat failed with `No handler registered for 'agent:send'`, Settings > Updates stuck on "Current Version: Loading…", Claude/OpenAI sign-in stuck "Checking…", and the misleading "install out of date" toast — surviving full uninstall/reinstall AND a hand-wiped `~/Library`. **Root cause (from a `tee`'d Terminal launch):** the beta.20 **arm64 mac assets shipped a Windows PE DLL as `better_sqlite3.node`** — `dlopen … not a mach-o file` → `SettingsManager.initialize()` threw → the old `app.whenReady` catch swallowed it → `setupIPC()` never ran → ZERO IPC handlers, window opens fine (renderer is static). Every Apple Silicon beta.20 install was bricked; Brett never saw it because he's on Intel x64 (healthy). Likely contamination path: Docker `dist:win` overwrote shared `node_modules` before the arm64 pack copied it. Confirmed in the local `release/mac-arm64` dir AND the published arm64 zip; beta.19/18 arm64 assets verified healthy. **Emergency mitigations (before the rebuild):** landing page Apple Silicon button pinned to beta.19 (`TSAI-Site@2307407`); arm64 entries stripped from beta.20's live `latest-mac.yml` via `gh release upload --clobber` so healthy arm64 beta.19 users wouldn't auto-update into the brick (verified in electron-updater 6.8.3 source: arm64 macs fall back to x64/Rosetta files by design). **Fixes shipped:** (1) `c9e1340` — `setupIPC()`/`setupUpdaterIPC()` now run FIRST in `app.whenReady`, before the throwable native-SQLite opens (registration only binds channels; handlers read state via self-guarding getters); init failures recorded in `startupError`, exposed via new `app:getStartupError` IPC + preload bridge, surfaced in a native `dialog.showErrorBox` AND by `ui/shared/ipc-error-handler.js` (which now shows the REAL error — the stale-install toast only fires when startup actually succeeded). (2) `28cbf9a`+`0866304` — `scripts/verify-native-modules.cjs` afterPack guard reads magic bytes of every `.node` in the packed app (Mach-O cputype vs target arch, PE, ELF; self-labeled multi-arch prebuilds like sharp/onnxruntime/canvas checked against their own path label; better-sqlite3's test-only `test_extension.node` ignored) and THROWS to fail the build; chained as the final step of `build/afterPack.cjs` — **note the gotcha: adding a second `"afterPack"` key in package.json silently loses (JSON dup key, last wins + sync-version rewrite dedupes)** — must chain inside the existing hook. Guard verified against the actual corrupt beta.20 arm64 pack (rejects, names better_sqlite3) and all healthy shipped packs (accepts). (3) `43850de` — `scripts/tester-rescue.sh` support script (refuses while app runs; chip-vs-app arch check; sqlite integrity check; timestamped DB set-aside, never deletes). **Release:** tag → `dist:signed` detached bg (both notarizations Accepted, DMGs stapled, `patch-latest-mac-yml` verified sha512s); because the running build pre-dated the guard chaining, both packs + the shipping arm64 zip were verified manually (extracted `better_sqlite3.node` = `Mach-O 64-bit bundle arm64`). Draft release with manifests first, 4 mac assets in 2 batches; **Windows not rebuilt** (beta.20 win assets healthy) — beta.20's `latest.yml` + all 3 setup.exe/blockmaps mirrored onto the beta.21 release so Windows updaters resolve against the newest release (version 1.0.0-beta.20 = no-op update for them). Published, all links verified. Landing page re-unified to beta.21 (`TSAI-Site@8a9baf6`). **Lessons:** wrong-platform native modules now fail the BUILD; never trust "reinstalled, same error" as ruling out the download itself; the "install out of date" toast was hiding three weeks of a far worse truth. **Open:** rotate arm64 testers onto beta.21; agent-performance upgrades (prompt-cache hygiene, tool-result truncation, compaction tuning) re-queued for beta.22; Campaign Ops Phase 2; carried-forward chores. |
| `v1.0.0-beta.20` | Jul 3, 2026 | **Twentieth beta — partner-reported spinner fix + security hardening pass.** Manny (Brett's testing partner) reported via `COS Bug.docx` that the chat's thinking indicator sometimes vanished while the agent was still working, and supplied a proposed patch. **Diagnosis verified against the backend before applying:** `chat-engine.ts` emits `done` after EVERY message (then `processQueue()` starts the next queued one), but the renderer's `done` handler tore the status indicator down unconditionally — so all subsequent `queue_processing`/`thinking`/`tool_start` events for queued messages hit `if (!statusEl) return;` and were silently dropped (agent working, UI idle, no stop button). **Fixes (`3378a6b`):** `ui/chat/message-renderer.js` — `done` cleanup now skips teardown while the session still has queued messages; a non-`done` status arriving with no on-screen indicator recreates it instead of dropping the event (currentSession guard runs first, per-session listener removal still bounds lifetime). `ui/chat/messaging.js` — both send-cleanup paths checked the GLOBAL `queuedMessageElements.size`, so a queued message in another tab blocked this session's cleanup (stuck-spinner variant); now checks session-scoped `queuedMessageIdsBySession`. Same class of bug in `stopQuery()`: global `queuedMessageElements.clear()` orphaned other sessions' queued tracking — now clears only the stopped session's ids. **Security hardening (from a full audit of Electron config, credential storage, IPC surface, renderer XSS paths):** (1) `52b33a3` — credential files owner-only 0600: `google-tokens.json` (Google refresh tokens) + `mcp-servers.json` (GHL private token, DataForSEO password in env) were default 644; writers now create 0600 and readers chmod pre-existing files down on first access (best-effort no-op on Windows). (2) `21ef090` — navigation guards: no `will-navigate`/`setWindowOpenHandler` existed anywhere; all windows via `createWindow()` now deny window.open and block navigation off `file://`, routing http(s) to `shell.openExternal` (browser-automation hidden window untouched — external nav is its purpose). (3) `acffa8d` — upstream's community chat server (`pocket-agent-chat-production.up.railway.app`, ws+http) removed from `ui/chat.html` CSP connect-src; Global Chat was already disabled/hidden, now CSP-enforced. **Audit findings that were already solid (no change):** contextIsolation/nodeIntegration correct on all windows; safeStorage encryption + plaintext migration; marked+DOMPurify with escape fallback; openExternal scheme-gated; extensive bash safety patterns. **gg-pixel audit:** documented in CLAUDE.md — transitive dep of pinned ggcoder@4.3.151, verified inert (only `DEFAULT_INGEST_URL` constant reachable; `install()` never called; pixel paths live in ggcoder's CLI which ACOS never runs). **Also fixed:** stale `ghl-node-server` test asserting 91 tools (beta.19's `d8e02e4` added `delete_contact` → 92, `1004cc0`). Suite 1248/1248, typecheck + lint clean. **Release pipeline ran clean:** AC_PASSWORD notarytool check passed pre-build; Mac `dist:signed` (detached bg) + Windows `dist:win` (Docker) in parallel; both DMGs notarized Accepted + stapled + validated; `patch-latest-mac-yml` verified 2 sha512 entries match disk; draft release with manifests/blockmaps first, 9 big assets in 5 batches, published — 18 assets. Landing page bumped (`TSAI-Site@a44e1c8`), Vercel auto-deployed. **Gotcha discovered while verifying:** TSAI-Site's edge proxy (`d833afd`, blocks scanners/probes) returns plain-text 404 to bare curl — looked like the whole site was down + `vercel domains add` misleadingly said "assigned to another project" (it was assigned to tsai-site all along, confirmed via API). **Verify the landing page with a browser User-Agent header, not bare curl.** Verified live serving beta.20, all download links HTTP 200. **Open / deferred:** agent-performance upgrades scoped for beta.21 (prompt-cache prefix hygiene, per-tool result truncation budgets, compaction threshold tuning); Campaign Ops Phase 2; autopost activation (Brett-gated); carried-forward chores from beta.16. |
| `v1.0.0-beta.19` | Jun 30, 2026 | **Nineteenth beta — ships the four unreleased workstreams (Silence Trimmer, Hook Lab, Video Studio, Campaign Ops Phase 1) after a full live click-through, plus full model selection.** Everything that had sat on `main` since beta.18 was finally exercised in the running app (dev build, on Brett's Claude/ChatGPT subscriptions — zero API keys) and two live ship-blocker bugs were found + fixed. **(1) Silence Trimmer** — `trim_video_silence` crashed instantly with `__dirname is not defined`: `src/tools/video-shared.ts` compiles to ESM (tsconfig `module: ES2022`) where CommonJS `__dirname` is undefined, so `resolveBundledAsset()` threw the moment the agent called the tool (and would have crashed Video Studio identically — same shared helper). Fixed by deriving `__dirname` from `import.meta.url`, matching `src/main/{windows,index,tray}.ts`. Verified: trimmed a real 8.27s clip → 5.74s, A/V drift 0.03s, original untouched (synthetic-TTS fillers weren't caught by the default Whisper model — documented limitation, not a bug). **(2) Video Studio** — the Aspect ratio card's 9:16/16:9/1:1 options were invisible: the card never got the `.expanded` class (`_vsRenderCard` handled workspace/brand/openai but aspect was rendered only by `_vsRenderAspect`, which toggled `.selected` not the body), so its `.vs-card-body` stayed at `max-height:0`. Fixed by expanding the card in `_vsRenderAspect`. Verified: rendered a real 9:16 1080×1920 10s H.264 MP4 to `~/Desktop/Videos/`, SKILL.md scaffolded to `~/dev/_video-studio/.agents/skills/remotion/SKILL.md`. **(3) Hook Lab** — passed clean, full 12-section hook system + 24/25 score + Lead-Gen Mode auto-fired. **(4) Campaign Ops Phase 1** — `campaign_smoke_test` passed all steps (create_contact, verify_tag) on Brett's real GHL sub-account `Uj6CJxWXVU8HyNgI39xb`; enroll skipped (no workflowId) and cleanup skipped (Brett's hand-managed Python `flo-ghl` server has no `delete_contact` — the vendored Node server subscribers use does, proven beta.16). The leftover synthetic contact `36UCLXzqlsfRZSMPuB5Y` was deleted out-of-band via the GHL API + verified gone. **(5) Full model selection (new feature).** New 'Check for new models…' option at the bottom of the model badge runs live discovery against Anthropic + OpenAI using the API key OR the subscription OAuth token (same auth shape gg-ai uses); best-effort + non-throwing so a token rejection/timeout just leaves the curated list intact. `src/agent/model-discovery.ts` (per-provider isolation, 15s timeout, OpenAI filtered to current-gen gpt-5+/codex/o-series — drops gpt-3.5/gpt-4 back-catalog + dated snapshots), `inferProviderFromId()` prefix routing in `providers.ts` (so a brand-new `claude-opus-4-8` / `gpt-5.6` routes correctly instead of defaulting to Anthropic), `getAvailableModels()` folds discovered models in (authed providers only, no dup curated ids) and sorts for display — providers grouped, families grouped, newest version first (Opus 4.8 → 4.7 → 4.6). Discovery cache replaces per-provider so stale ids drop out. `settings:discoverModels` IPC + preload bridge. Verified live: discovery surfaced **Opus 4.8** + **Fable 5**; Opus 4.8 confirmed actually running via per-turn log `Session config — model: claude-opus-4-8, provider: anthropic`; Fable 5 returns Anthropic's own `404` (gated upstream — will work when access opens, no app change). Typecheck + lint clean throughout. **Subscription-auth side proof:** every test ran on Brett's Claude Pro/Max + ChatGPT subscriptions with no API keys — confirming the existing OAuth login path works end-to-end across all features. **Open / deferred:** Campaign Ops Phase 2 (Google Ads connector + ad-spend→GHL stats join); autopost activation (Brett-gated); the niche providers (Kimi/GLM/Xiaomi/MiniMax/DeepSeek) stay curated since they have no usable models endpoint; carried-forward chores from beta.16. |
| `v1.0.0-beta.16` | Jun 3, 2026 | **Sixteenth beta — GoHighLevel tester-connect (Python-free) + chat tab cap.** Two shipped changes. **(1) GHL connect with no Python, now on Windows.** Reimplemented the 91-tool GoHighLevel sub-account MCP server as a vendored Node ESM server (`vendor/ghl-mcp-node/index.js`) spawned via Electron's bundled Node (`process.execPath` + `ELECTRON_RUN_AS_NODE=1`), mirroring the Flo servers — removes the Python runtime that blocked clean-install testers and hid GHL on Windows. Ported 1:1 from `vendor/ghl-mcp/main.py`: identical tool names/args/REST endpoints/bodies, `Version: 2021-07-28` header, `locationId` injection, 25k GET truncation, `HTTP <code>: <body[:500]>` error passthrough; Node 18+ global `fetch` (no httpx), 30s `AbortSignal` timeout. SDK `@modelcontextprotocol/sdk@^0.5.0` vendored into `ghl-mcp-node/node_modules` (pure JS, no native modules → no afterPack symlink, no better-sqlite3 seal impact). `connect-tools-ipc.ts` GHL entry now Node-spawns (was `python3`); `unavailableOnWindows` removed; Brett's hand-managed `flo-ghl`/`flo-ghl-brett` venv aliases preserved for status. New `resolveGhlNodePath` in `bundled-paths.ts`. Python `vendor/ghl-mcp/` kept on disk as reference only (no longer referenced by code — prune in a later cleanup). Tests: updated `bundled-paths` + `connect-tools-ipc` suites, new `ghl-node-server` mapper suite; `npm test` green (1219 tests), typecheck + lint clean. **Verified live against a real GHL sub-account** (`OfcMDEmwDKM6qQZahiuf`): 91 tools served, `get_pipelines` read returned the real "Master's Edge" pipeline, `create_contact` write succeeded, and the same proof ran **from inside the signed/notarized x64 .app** (confirming `vendor/ghl-mcp-node/` incl. its vendored SDK survives code-signing). On Brett's own machine the GHL card shows "hand-managed" because his pre-existing `flo-ghl` alias lacks `_acos_managed` — a fresh tester with no prior entry gets empty fields and connects normally (tested by writing the canonical `ghl-mcp` Node entry directly, the exact shape `buildEntry` produces). **(2) Chat tab cap 10 → 20** with a fix: at the cap the "+ New Chat" button no longer disappears (which looked like a regression) — it stays visible but dimmed with a 'Max 20 chats open' tooltip, and clicking it (or a shortcut) shows a bottom-right Notyf toast. **Build pipeline:** `dist:signed` produced both arch .apps notarized successfully, but the `afterAllArtifactBuild` DMG-staple step failed first pass with `No Keychain password item found for profile: AC_PASSWORD` — the documented transient (login-keychain lookup blip; the profile + history were intact, confirmed via `xcrun notarytool history --keychain-profile AC_PASSWORD`). Fixed without rebuilding: stapled both DMGs in place (`notarytool submit --wait` + `stapler staple`, both Accepted), then **re-ran `patch-latest-mac-yml.cjs`** so `latest-mac.yml` sha512/size match the post-staple DMG bytes (else auto-update checksum mismatch). **Lesson for next release: confirm the `AC_PASSWORD` profile resolves BEFORE kicking off `dist:signed`** (one-line `notarytool history` check). `dist:win` via Docker built signed universal + x64 + arm64 NSIS. `gh release create` published prerelease with manifests + blockmaps, then large artifacts uploaded in batches (the 641MB universal `setup.exe` ran past the foreground timeout — uploaded in background); all 16 beta.16 assets present. Landing page (`TSAI-Site/app/hidden/ai-chief-of-staff-app/page.tsx`) bumped beta.14 → beta.16 (note: beta.15 was never reflected there), Vercel auto-deployed, verified live serving v1.0.0-beta.16 with download URLs resolving (DMGs HTTP 200). **Open / deferred:** live Windows pass (code-clean, no Windows hardware — same posture as voice); prune the reference-only Python `vendor/ghl-mcp/` tree once the Node port is field-proven; rotate the GHL test tokens used this session; carried-forward from beta.14 — install:local symlink seal (Goal `b6168502`), tray single-click flake, Connect Tools card collapses too fast to read, upstream the Flo `oauth.js` env-var patch. |
| `v1.0.0-beta.14` | May 29, 2026 (morning) | **Fourteenth beta — morning tester-bug fixes + build reproducibility.** Shipped after a live testing session with Brett on the installed app. **Two tester-reported bugs investigated + fixed:** (1) **Content Writer falsely reported "DataForSEO key needed"** even though credentials were present and the server ran with 83 tools. Root cause: the May 28 recovery session renamed the MCP entry `dataforseo` → `dataforseo-mcp-server` (so its Connect Tools card would resolve), which silently broke `content-writer-panel.js`'s exact-name lookup (`s.name === 'dataforseo'`). Fix (`5ce6b32`): `_cwIsDataForSEOServer()` detects the connection rename-proof — by the `DATAFORSEO_USERNAME`/`PASSWORD` env var first, then known package name in command/args, then historical names; the save path updates the existing entry in place (preserving its name) instead of duplicating. Verified live — card went green, blog generated end-to-end. (2) **Voice input wrote a phantom "you" and nothing else.** Misdiagnosed twice before the real cause was found in the macOS TCC system log: the morning's UI hot-copies into the installed `.app` had **invalidated the code signature**, so the hardened-runtime mic entitlement (`com.apple.security.device.audio-input`) was treated as missing — macOS silently DENIED mic access (`Policy disallows prompt`) and fed Chromium a SILENT stream, which Whisper transcribed as "you." The voice code was never broken. **Resolved by re-signing the installed bundle in place** with the Developer ID + a resolved entitlements plist (the `$(AppIdentifierPrefix)` was expanded to the real `2HQTY95NHD.` team prefix so safeStorage's keychain-access-group stayed valid), inside-out (nested binaries → Electron Framework → helpers → app), then `tccutil reset Microphone com.totalsuccessai.ai-chief-of-staff`. Verified: `.app` → "accepted / Notarized Developer ID", mic prompt appeared, voice transcribed correctly. **Code fixes from this incident:** mic tooltip no longer says "needs OpenAI key" once visible (`b61060e`); voice silence guard hardened with a bytes-per-second heuristic (>=600ms recorded but <1200 B/s avg = treat as silence, honest "check mic permission" toast) so a future denied/muted mic never produces a phantom "you" (`fd49e62`). **Build hygiene this round:** pinned `@kenkaiiii/gg-agent` + `@kenkaiiii/gg-ai` to exact `4.3.151` and removed `prebuild`'s `npm update @kenkaiiii/...` (the long-standing footgun — builds were non-reproducible) (`6fa0f41`); fixed 2 pre-existing eslint errors carried in the beta.14 batch (`NodeJS.ErrnoException` → inline narrowing; empty `ConnectPayloadGoogle` interface → `Record<string, never>`) (`1209d88`); added `RESTORE-PLAN-*.md` / `ACOS-RESUME-*.md` to `.gitignore` (transient working notes stay local) (`3d40fe8`). **Also carried forward from the uncommitted May 28 batch** (committed before this session): Google OAuth `select_account consent` chooser (`731c94d`), Connect Tools cards derive from live MCP server not ACOS OAuth state (`0ce257b`), workspace write-probe + non-synced fallback for the iCloud EPERM (`cab95d6`), OAuth test (`1b58adf`). **Release pipeline:** `dist:signed` ran as a managed background process (after two earlier runs were reaped mid-notarization leaving DMGs unstapled — see build reminders above); both arch `.app`s + both DMGs notarized + stapled + validated, `latest-mac.yml` patched to beta.14. `dist:win` via Docker built signed x64 + arm64 + universal NSIS. `gh release create` timed out at 6/18 assets (left a DRAFT) — uploaded the remaining 12 in batches via `gh release upload`, then `gh release edit --draft=false` to publish. Telemetry guard clean; LICENSE (© KenKaiii) + README upstream attribution intact. Landing page bumped to beta.14 (`TSAI-Site@9083a01`), Vercel auto-deployed, verified live serving v1.0.0-beta.14 with download links resolving (200/302). **Brett's local app was on the hot-copied + re-signed beta.13 bundle at session end** — will auto-update to clean signed beta.14 on next quit/reopen cycle (`autoDownload=true`). **Still open (deferred to beta.15+):** cron-stop bug (Step 11 git-push silent abort — still hypothesis stage); tray single-click flake; upstream the Flo `oauth.js` env-var patch into `~/flo-assistant/shared/src/oauth.ts` + retire the vendor fork; the **install:local symlink seal-breaking Goal `b6168502`** (vendor `better-sqlite3`/`bindings`/`file-uri-to-path` symlinks fail `codesign --deep --strict`, so re-signing a local build isn't notarization-grade — only matters for install:local, NOT the `dist:signed` pipeline which adds them in afterPack before signing); Google verification at 80 testers; DMG size from bundled googleapis. |
| `v1.0.0-beta.12` | May 25, 2026 (evening) | **Twelfth beta release — Connect Tools marketplace + Google OAuth + voice fixes + Content Writer social spin.** The May 23 code drop (Connect Tools marketplace, bundled Flo MCP servers, Google OAuth) finally ships to testers. New sidebar button between *The Brain* and *Settings* opens a 7-card marketplace (Gmail, Calendar, Drive, Bookmarks, GHL, DataForSEO, Firecrawl). The Google card runs PKCE Installed-App OAuth against the new `tsai-ai-chief-of-staff` Google Cloud project; tokens persist to `<userData>/google-tokens.json` with proactive 5-min-leeway refresh. Bundled flo-gmail / flo-calendar / flo-docs / flo-bookmarks MCP servers (with the new `gmail_get_message` tool) ship inside the .app via `vendor/flo-mcp-servers/`; the beta.9 onboarding mockup `ob-step-connectors` is now a real OAuth flow. **Security hardening this round**: real Google OAuth secret rotated in Google Cloud Console (old `…Nva` disabled by Brett, new `…pkYA` active); secrets moved out of source via new `scripts/inject-google-credentials.cjs` build-time substitution that reads `.env.production` (gitignored) or env vars; release builds (`dist:signed` / `dist:win`) set `BUILD_KIND=release` so the injector refuses to ship placeholder credentials. Old secret scrubbed from git history via `git-filter-repo --replace-text` (backup branch `backup/pre-secret-scrub-20260525`). **UX polish in this release**: composer bottom-bar gained tooltips for background-tasks toggle + mode-select + model-badge (commit `0c7d04b`); Connect Tools panel no longer stacks a yellow hand-managed warning over a red error — they fold into one actionable message (commit `9fe2b2a`). Voice input fixes from the unshipped feature branch finally land: green mic colour, live interim transcription text, killed the 'you' hallucination at the end of recordings. Content Writer social spin: after the agent posts a blog draft inline, **✓ Approve & Publish** writes the .md to `~/Desktop/Blogs/{slug}/`; **⚡ Create social content** generates 5 platform-tailored posts (GBP / Facebook / Instagram / LinkedIn / Medium) + a 1080×1080 Instagram square image alongside the blog. **gh hygiene** (same session): `gh auth status` had 4 logged-in accounts with PMMARocks-1 active (READ on the repo — would have blocked any `gh release create`); switched active to BrettLechtenbrerg (ADMIN), logged out the 3 unrelated accounts so the wrong one can't be picked accidentally. **Release pipeline ran clean**: Mac DMGs signed + notarized + stapled in parallel with the Docker Windows build (both started ~18:23, both done by 18:50). `scripts/patch-latest-mac-yml.cjs` caught + patched 2 stale entries on first run (same fix path as beta.10/.11). `gh release create` uploaded all 18 assets in one call — no mid-upload timeout. Landing page bumped (`TSAI-Site@57823d2`), `vercel --prod` deployed live, beta.7+ installs auto-pull on next quit. **What's NOT in this release** (deferred): cron-stop bug fix (Step 11 git-push silent abort — still hypothesis stage), tray single-click flake, `@kenkaiiii/gg-agent` + `gg-ai` version pinning. **Verified post-rotation**: old secret returns HTTP 401 `invalid_client` against Google's token endpoint after ~60s propagation; new secret returns HTTP 200 with full 6-scope grant against the dev Mac's existing refresh token (no re-OAuth needed for already-connected accounts). **Brett's `~/.flo/` setup (powering Claude Desktop) is fully independent of ACOS** — different Google Cloud project (`441826313373-…`), different credential files, different token file. Tested live: Flo refresh-token → 200 OK against `flo-assistant` client; ACOS work doesn't touch any of those paths. |
...
- **New:** `assets/skills/video-silence-trimmer/SKILL.md` + `trimmer.py` (the working Python skill: ffprobe video/audio detect → ffmpeg audio extract → word-level transcription → mark filler + silence removals → complement into keep segments w/ padding → single ffmpeg `filter_complex` trim+concat pass for A/V sync → temp-file-then-move so the input is never clobbered). `src/tools/video-trim.ts` (`trim_video_silence` tool: resolves the bundled script, defaults output to `~/Desktop/Trimmed/`, refuses `.app` paths, parses the script's final JSON stdout line, surfaces the script's own dependency-install errors).
- **Changed:** `src/tools/video-shared.ts` (generalized `resolveBundledSkill` → added `resolveBundledAsset(...segments)`; the remotion resolver now delegates to it — low-risk refactor), `src/tools/index.ts` (registers the tool next to the Video Studio tools).
- **Transcription engines:** `faster-whisper` default (on-device, no API key; `WHISPER_MODEL` env, default `base`), with `openai` (OPENAI_API_KEY) and `elevenlabs` (ELEVENLABS_API_KEY) fallbacks. CLI args mirror the doc: `--input --output --silence-threshold --filler-words --padding --engine`.
- **EXTERNAL deps, nothing bundled:** needs ffmpeg + the chosen transcription package on the USER's machine — same philosophy as Video Studio's external Remotion. The tool self-gates: the script emits a clean JSON error with install guidance (`brew install ffmpeg`, `pip install faster-whisper`) when a dep is missing. No app-size/notarization impact; only `SKILL.md` + `trimmer.py` ship via `assets/**`.
- **No panel** — unlike Video Studio/Hook Lab, this is a one-shot utility, so it's a tool the agent calls on request ("trim the silence out of this video"), pairing naturally with Video Studio renders. (A panel/button can be added later if Brett wants a one-click surface.)
- **Verification done:** typecheck + lint clean, `py_compile` clean; segment math unit-tested (filler+silence removal, keep-segment complement w/ padding edge cases, filter_complex shapes) and dependency-error/validation paths confirmed to emit clean JSON with distinct exit codes.
- **LIVE E2E PASSED (Jun 29):** installed ffmpeg 8.1.2 (`brew`) + faster-whisper 1.2.1 (`pip`), built a synthetic TTS test clip (speech → 1.5s silence → "um" → speech → "uh" → 2.0s silence → speech) as both `.mp4` (640x360 video) and `.wav` audio-only. Ran the bundled `trimmer.py` directly: both produced valid output, **A/V drift 0.033s (in sync)**, `overwroteInput:false` on default runs, and an explicit same-output-path run correctly trimmed in place (`overwroteInput:true`). The audio-only input produced an audio-only output (no phantom video stream). The script's JSON keys match exactly what `video-trim.ts` parses. **Caveat (documented in SKILL.md):** filler-word *accuracy* depends on the Whisper model — the `base` default removed 1 of 2 synthetic "um/uh" and absorbed one silence into a word's duration; `WHISPER_MODEL=small` caught both silences. The trim LOGIC is correct — it removes exactly what the transcript surfaces; accuracy scales with model size. The whisper `base`+`small` models are now cached in `~/.cache/huggingface` on this Mac.
- **Next steps:** the only thing not exercised is the in-app path (agent calling `trim_video_silence` live) — `npm run rebuild:native` + relaunch, then ask the agent to trim a clip. Logic + script are proven; this is the same GUI-click-through step the other workstreams await. Then releasable.
