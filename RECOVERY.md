...
### Next session — pick up here (added May 19, end of day)

**Status: massive day shipped. Beta.10 code is on Brett's local Mac but NOT yet on a signed/notarized DMG nor on GitHub Releases. Next session = cut the beta.10 release.**

**What shipped today (May 19), in chronological order:**
...
**Beta.10 release — what's left:**

- Local `/Applications/AI Chief of Staff.app` is on beta.10 (unsigned `dist:install` build).
- Tag `v1.0.0-beta.10` is on origin.
- **NOT yet done:** signed/notarized Mac DMG (`npm run dist:signed`), Windows installers (Docker `dist:win:local`), GitHub Releases upload of 11 artifacts, landing page bump in `TSAI-Site`, `vercel --prod` deploy of the landing page. Testers on beta.8/beta.9 don't have today's fixes yet.
- Carryover follow-ups from beta.9 (still open): pin `@kenkaiiii/gg-agent` + `gg-ai` versions to stop the prebuild auto-update footgun; move `latest-mac.yml` YAML patcher out of the `afterAllArtifactBuild` hook into a separate post-build npm script; tray-click single-click flake.

**First action next session:** cut and ship the signed beta.10 release. ~25 min of mostly waiting on the build pipeline.
