...
> **Latest release**: https://github.com/BrettLechtenbrerg/ai-chief-of-staff/releases/tag/v1.0.0-beta.6 (public, prerelease)
...
### Now
- **`v1.0.0-beta.6` shipped (May 17, night).** All 13 day's-worth of fixes published as a single prerelease — see the v1.0.0-beta.6 row in the rollback table. Mac DMGs signed/notarized/stapled, Windows installers built, all 11 artifacts on GitHub Releases, landing page bumped + deployed. Local `/Applications/AI Chief of Staff.app` running beta.6. Auto-updater on beta.1–beta.5 installs pulls this build on next launch. Carried-over follow-ups to clean up before beta.7:
  - Tray single-click → chat is still flaky; tracked in task list (id `9f91cdd0`).
  - `build/afterAllArtifactBuild.cjs` `latest-mac.yml` regex still flaky — had to hand-patch post-staple sha512+size for beta.6 like beta.5. Needs a real fix so the post-staple patcher is automatic.
  - Apple notarization `AC_PASSWORD` keychain profile worked clean for beta.6 (`xcrun notarytool history` confirms), but had transient failures on previous `dist:install`s. Monitor; recreate with `xcrun notarytool store-credentials` if it regresses.
  - External MCP server patches (`~/ghl-mcp/main.py`, `~/flo-assistant/servers/*/dist/index.js`) live in Brett's local clones — NOT bundled in the ACOS DMG. Acceptable for now (testers without those servers don't trigger those bugs), but document this in tester README if MCP servers ever ship as part of the install.
