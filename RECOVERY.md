...
### Now
- **Mid-cycle bug-fix pass complete (May 17)** — see `v0.6-tooling-and-fixes` row in the rollback table. Eight changes shipped against `1.0.0-beta.5` without a version bump: tray UX, panel tooltips, edit-task button, navy-on-silver button text, GHL `skip→page` across 10 endpoints, Flo calendar recurring path (execute branch + timezone), system-prompt Tool Discipline rules to stop curl-workaround behavior, and the install/hot-copy workflow refined. **Next public release should be `v1.0.0-beta.6`** — bump `package.json` + tag + signed build + publish DMGs/EXEs + bump landing page `RELEASE_TAG`. Open follow-ups before cutting beta.6:
